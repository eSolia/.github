...
First create the docs/checkpoints directory if it doesn't exist, then create the checkpoint file.

After creating the file, run a formatting/lint check if tools are available, then `git add` and `git commit` only the checkpoint file. Do NOT push — let the user decide.
